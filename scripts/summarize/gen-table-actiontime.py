...
# This scripts compiles a table in CSV or LaTeX format of the average/median time
# per action across experimental runs.
    ...
    with open(args.infile, "r") as csvfile:
        ...
        for row in reader:
            if row["Condition"] == condition:
                data.append( int(row["Runtime"]) / int(row["Actions"]) )
    return data
